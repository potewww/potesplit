...
function populateFormSelects() {
  ...
  selects.forEach(sel => {
    ...
    sel.innerHTML = STATE.persone.map(p => `<option value="${p}">${p}</option>`).join("");
  ...
  checkboxContainers.forEach(box => {
    ...
    box.innerHTML = STATE.persone.map(p =>
...
function modificaCena(i) {
  ...
  document.querySelector("#cena-persone-rows").innerHTML = "";
  document.querySelector("#cena-spese-rows").innerHTML = "";
  cenaPersoneRows = []; cenaSpeseRows = [];
  c.persone.forEach(p => addCenaPersonaRow(p));
...
async function submitPersona(e) {
  e.preventDefault();
  const nome = document.querySelector("#f-persona-nome").value.trim();
  if (!nome) return;
  if (STATE.persone.includes(nome)) { setStatus("#status-persona", "Questa persona esiste già.", true); return; }
  try {
    const nuovoElenco = [...STATE.persone, nome];
    await GH.writeJSON("data/persone.json", nuovoElenco, `Aggiunge persona: ${nome}`);
    setStatus("#status-persona", "Persona aggiunta! Ricarico i dati…", false);
    document.querySelector("#f-persona-nome").value = "";
    await loadAllData(); renderAll();
...
function addCenaPersonaRow(initial) {
  const idx = cenaPersoneRows.length;
  cenaPersoneRows.push(idx);
  const box = document.querySelector("#cena-persone-rows");
  const row = el("div", "cena-persona-row");
  row.dataset.idx = idx;
  row.innerHTML = `
    <div class="cp-field cp-field-nome"><label>Persona</label><select class="persona-select cp-nome"></select></div>
    ${CAT_INPUT.map(c => `<div class="cp-field"><label>${CAT_LABELS[c]}</label><input type="number" step="0.01" class="cp-${c}" value="0"></div>`).join("")}
    <div class="cp-field"><label>Pagato</label><input type="number" step="0.01" class="cp-pagato" value="0"></div>
    <button type="button" class="btn-remove-row" title="Rimuovi persona">✕</button>`;
  box.appendChild(row);
  populateFormSelects();
  if (initial) {
    row.querySelector(".cp-nome").value = initial.nome;
    CAT_INPUT.forEach(c => { row.querySelector(`.cp-${c}`).value = initial[c] || 0; });
    row.querySelector(".cp-pagato").value = initial.pagato || 0;
  }
  row.querySelector(".btn-remove-row").addEventListener("click", () => {
    row.remove();
    cenaPersoneRows = cenaPersoneRows.filter(i => i !== idx);
  });
}
...
function addCenaSpesaRow(initial) {
  ...
  row.innerHTML = `
    ...
    <div class="persona-checkboxes cs-part"></div>
...
async function submitCena(e) {
  ...
  const persone = Array.from(document.querySelectorAll("#cena-persone-rows .cena-persona-row")).map(row => {
    const p = { nome: row.querySelector(".cp-nome").value };
    CAT_INPUT.forEach(c => p[c] = parseFloat(row.querySelector(`.cp-${c}`).value) || 0);
    p.pagato = parseFloat(row.querySelector(".cp-pagato").value) || 0;
    return p;
  });
  if (persone.length === 0) { setStatus("#status-cena", "Aggiungi almeno una persona.", true); return; }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  aggiornaCampiCredenziale();
  document.querySelector("#btn-add-cena-persona").addEventListener("click", () => addCenaPersonaRow());
